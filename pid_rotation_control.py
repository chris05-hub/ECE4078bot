...
def pid_control():
    # Applies PID for both forward/backward AND rotation movements
    ...
    while running:          
        ...
        if not use_PID:
            ...
            if current_movement == 'forward' or current_movement == 'backward':
                ...
                proportional = KP * error
                rotation_integral += KI * error * dt
                rotation_integral = max(-MAX_CORRECTION, min(rotation_integral, MAX_CORRECTION))  # Anti-windup
                derivative = KD * (error - rotation_last_error) / dt if dt > 0 else 0
        ...
        if ramp_left_pwm != 0 or ramp_right_pwm != 0: # print for debugging purpose
